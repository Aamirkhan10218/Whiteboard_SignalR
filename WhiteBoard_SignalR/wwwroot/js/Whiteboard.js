...
//const { signalR } = require("./SignalRCore3.0");
...
var con = new signalR.HubConnectionBuilder().withUrl("/whiteboardHub").build();
var tools = {};
var tool;
var WIDTH;
var HEIGHT;
...
function DrawIt(drawObject, syncServer) {
    ...
    if (syncServer && drawObject.Tool != DrawTool.Pencil) {
        ...
        con.invoke("sendDraw", message, $("#sessinId").val(), $("#groupName").val(), $("#userName").val());
    }
}
...
function JoinHub() {
    ...
    $("#btnJoin").click(function () {
        ...
        if (name.length > 0) {
            ...
            con.on("handleDraw", function (message, sessnId, name) {
            ...
            con.on("chatJoined", function (name) {
            ...
            con.on("chat", function (name, message) {
            ...
            //    () => con.invoke("joinGroup", $("#userName").val(), $("#groupName").val())
            //);
            con.start().catch(function (err) {
                return console.error(err.toString());
            });
            con.invoke("joinGroup", $("#groupName").val());
            ...
            $("#btnSend").click(
                function () {
                    ...
                    if (message.length > 0) {
                        ...
                        con.invoke("sendChat", message, $("#groupName").val(), $("#userName").val());
                        $("#txtMessage").val("");
                    }
                }
            );
